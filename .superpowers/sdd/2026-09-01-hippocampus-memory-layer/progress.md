...
- Task 4 privacy-deletion follow-up closes the historical keyframe leak: single-event, range, retention, and full-brain deletion unlink last-reference encrypted blobs. Every retention cycle, including `forever`, also reconciles canonical crash orphans and stale writer temp files after a one-hour grace period without following symlinks or touching unknown entries. The focused FFI path, strict Clippy, and full `mci-brain` plus `mci-agent` suites pass; the explicitly required 30-minute capture soak remains open.
- Task 8 clean-home engine E2E now passes from a throwaway `HOME`: capture defaults off, 20 deterministic events seed, a shared-encoder `OCREvent` traverses production `--drain-stdin --strict`, enrichment creates episodes, the brief row is read back, MCP recall/timeline/episodes/cited context resolve the injected event, Claude and Codex receive reference-only custody, deletion returns the brain to 20 rows, and uninstall leaves no isolated product state. Release CI runs both the source contract and executable proof. Actual `.app` assembly/launch remains the next gate.
