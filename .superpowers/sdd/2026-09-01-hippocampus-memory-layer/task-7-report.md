...
Final review repair: `fix: model brief capture coverage explicitly` (the commit containing this updated report, based on `f6f2b30`; exact SHA is returned in the Task 7 handoff).
...
- Replaced the Brief coverage Bool with `CaptureCoverage.unknown`, `.insufficient(captureHoursSoFar:)`, and `.fullDay`; MemoryWorkspace passes `.unknown` because Recall has no trustworthy coverage provider.
- Added a distinct `.captureCoverageUnknown` scene and neutral copy so unknown is not presented as stopped, failed, insufficient, or complete.
- Retained explicit Bool compatibility initializers for callers/tests that really possess a source-backed threshold measurement; `false` maps to `.insufficient` and `true` maps to `.fullDay`.
...
- `apps/recall-ui/Sources/RecallUIKit/BriefViewModel.swift`
...
- `apps/recall-ui/Tests/RecallUIKitTests/BriefViewModelTests.swift`
...
- Final-review RED: a direct contract typecheck failed because `CaptureCoverage` and `BriefScene.captureCoverageUnknown` did not exist.
- Added `testReloadWithUnknownCoverageUsesNeutralUnknownScene`, which loads an empty brief store with `.unknown` and requires the neutral unknown scene.
- Final-review GREEN: the focused executable probe passed unknown, measured-insufficient, and full-day branches, including both Bool compatibility mappings.
...
- Final coverage repair `swiftc -parse` over `BriefViewModel.swift`, `BriefView.swift`, `MemoryWorkspaceView.swift`, and `BriefViewModelTests.swift`: passed.
- Final coverage repair direct RecallUIKit and RecallUI typechecks at `arm64-apple-macosx14.0`: passed with only the previously recorded warnings.
- Direct three-state coverage executable: passed with `Task 7 capture coverage probe passed`.
...
- Production-source scan found no hard-coded `hasFullDayCapture: true` or `hasFullDayCapture: false`; the legacy label remains only on compatibility initializer parameters and source-backed tests.
- Prior forbidden regression scan remains clean for the `hits.count` source label, OCR fallback in the keyframe filter, hardcoded `⌘6` custom-names shortcut, capture footer, capture-value derivation, and `showsCaptureStatus` metadata.
...
- Brief model availability is backed by the existing on-disk model probe. Capture coverage is a typed three-state value; unknown, measured-insufficient, and full-day results render distinct scenes.
