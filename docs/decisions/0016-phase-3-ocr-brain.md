...
2. **Cascade-twice for OCR.** The cascade runs on pixels at frame time (§1–§5 + §7); on `.allow` the OCR worker runs; **the cascade §6 (OCR-time secret/PII regex; SecretBench-tuned per ADR-0013 §6) re-runs against the OCR'd text**. Only events that clear BOTH cascades become `OCREvent` payloads on the wire and reach the store. Events that clear pixels but fail §6 on text become `PrivacyTombstone(reason=6)` instead — no OCR'd text bytes cross the wire, no embedding is computed, no event row is inserted.
