...
// AX outcome enum + cascade decision + OCR result length
// (no content)."
//
// PROTECTED-SET per AGENT_PROTOCOL §5. Content-free by construction:
// the only value surfaces are fixed enums, booleans, and integer counts.
// NEVER bundle ID, recognized text, window title, identifier, or URL.
//
...
public enum OCRTrace {
    ...
    /// caller-supplied; do NOT pass bundle IDs, OCR text, or any other
    /// user-visible string.
