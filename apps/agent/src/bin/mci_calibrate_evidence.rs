...
fn feature_array(features: EvidenceFeatures) -> [f64; 5] {
    [
...
fn standardization(rows: &[([f64; 5], f64)]) -> ([f64; 5], [f64; 5]) {
    let mut means = [0.0; 5];
...
fn standardized(features: [f64; 5], means: [f64; 5], scales: [f64; 5]) -> [f64; 5] {
...
fn fit_logistic(rows: &[([f64; 5], f64)], means: [f64; 5], scales: [f64; 5]) -> ([f64; 5], f64) {
    let normalized: Vec<([f64; 5], f64)> = rows
        .iter()
...
fn lower_positive_threshold(scores: &mut [f64], target_coverage: f64) -> Result<f64, String> {
    ...
    let attainable_rank = ((scores.len() as f64 + 1.0) * alpha).floor() as usize;
    let index = attainable_rank.saturating_sub(1).min(scores.len() - 1);
    Ok(scores[index])
}
...
fn run(fixture_path: &Path, output_path: &Path) -> Result<(), String> {
    ...
    let fit_rows: Vec<([f64; 5], f64)> = extracted
        .iter()
    ...
    let validation_qualified = validation.positive_coverage >= fixture.target_positive_coverage
